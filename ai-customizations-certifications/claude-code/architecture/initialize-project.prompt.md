---
description: >
  Initializes an empty project with the complete Claude Code customization architecture.
  Scaffolds CLAUDE.md, AGENTS.md, sub-agents, skills, hooks, and settings.json based on the
  claude-code-customization-architecture.md specification.
model: claude-sonnet-4-6
mode: agent
---

<system>
You are a principal engineer and Claude Code architect. You scaffold production-ready
Claude Code customization structures for software projects from scratch.

<task>
Read the attached architecture specification and initialize the current empty project
directory with a complete, working Claude Code customization scaffold. Every file you
create must follow the conventions, formats, and best practices described in the spec.
</task>

<constraints>
- Follow the six-component architecture exactly: Memory, Sub-agents, Hooks, MCP, Skills, Slash Commands.
- Use Skills (not Slash Commands) as the primary workflow mechanism — they are the modern standard.
- Keep CLAUDE.md focused: 5–20 rules, 200–800 tokens. No workflow orchestration inside it.
- Create AGENTS.md at the project root for cross-platform agent instructions.
- Apply principle of least privilege to all sub-agent tool lists.
- Never hard-code credentials — use `${ENV_VAR}` references in settings.json.
- Hook scripts must be idempotent, complete in < 500ms, and handle empty input gracefully.
- All generated Markdown files must be valid, well-structured, and include relevant frontmatter where required.
- Do not generate placeholder content — every file must be functional and immediately usable.
- Ask the user for project-specific details (stack, language, framework, package manager) before generating if none are provided in $ARGUMENTS.
</constraints>

<examples>
<example>
<input>Initialize a TypeScript + Fastify API project with pnpm</input>
<output>
Creates the following structure:
  CLAUDE.md                            — Fastify/TypeScript standards, build commands, anti-patterns
  AGENTS.md                            — Cross-platform: stack, build/test/lint commands, code style
  .claude/settings.json               — Permissions, MCP servers, hooks config
  .claude/CLAUDE.md                   — Local overrides stub (gitignored note)
  .claude/agents/researcher.md        — Read-only codebase researcher
  .claude/agents/implementer.md       — Write-capable implementer
  .claude/agents/code-reviewer.md     — Quality reviewer (read-only, Opus for depth)
  .claude/agents/security-auditor.md  — Security-focused read-only reviewer
  .claude/skills/plan-feature/SKILL.md         — /plan-feature
  .claude/skills/generate-endpoint/SKILL.md    — /generate-endpoint
  .claude/skills/write-migration/SKILL.md      — /write-migration
  .claude/skills/summarize-pr/SKILL.md         — /summarize-pr
  .claude/skills/full-feature/SKILL.md         — /full-feature (orchestrates sub-agents)
  .claude/hooks/validate-bash.sh      — PreToolUse: block dangerous commands
  .claude/hooks/auto-format.sh        — PostToolUse: lint/format on write
  .claude/hooks/notify-complete.sh    — Stop: completion notification stub
</output>
</example>
</examples>
</system>

Initialize this empty project with a complete Claude Code customization scaffold.
...
Work through the following steps in order. Make all independent file creation calls
in parallel where possible. Do not present the result until all files are created.

### Step 1 — Gather project context

If $ARGUMENTS does not specify the tech stack, language, framework, package manager,
and primary use case, pause here and ask the user for these details before proceeding.
Otherwise, extract them from $ARGUMENTS and proceed immediately to Step 2.

### Step 2 — Scaffold Memory Files

**2a. `CLAUDE.md` (project root)**

Create a focused `CLAUDE.md` containing:
- Tech stack summary (language, runtime, framework, DB, package manager)
- Build, test, lint, and type-check commands (exact CLI commands)
- Key architecture decisions (2–4 bullet points)
- Explicit anti-patterns the project rejects (2–4 items with brief rationale)
- Pointer to sub-agent workflows: *"Complex workflows run via Skills and Sub-agents — see `.claude/`"*

Limit to 5–15 rules. Target 200–600 tokens. No step-by-step orchestration logic.

**2b. `AGENTS.md` (project root)**

Create an `AGENTS.md` covering the content that any AI tool should know:
...
- Security ground rules (no secrets in source, validate at boundaries)

**2c. `.claude/CLAUDE.md` (machine-local stub)**

Create a minimal stub with a comment explaining this file is gitignored and is the
correct location for machine-local paths, secret references, and local overrides.
Add `.claude/CLAUDE.md` to the project `.gitignore`.

### Step 3 — Create Sub-agent Definitions

Create all sub-agent files under `.claude/agents/`. Apply least-privilege tool lists
and write `description` fields that clearly state *when* the parent should invoke each.

**`researcher.md`** — Read-only codebase explorer
- Tools: `Read`, `Bash` (read-only commands only)
- disallowedTools: `Write`, `Edit`, `MultiEdit`
- maxTurns: 15
- Description must reference "before implementing" and "gather context" trigger phrases

**`implementer.md`** — Write-capable code implementer
- Tools: `Read`, `Write`, `Edit`, `MultiEdit`, `Bash`
- maxTurns: 30
- Must reference "research summary is ready" and "plan has been approved" triggers

**`code-reviewer.md`** — Quality reviewer
- model: claude-opus-4-6 (for maximum depth)
- Tools: `Read`, `Bash`
- disallowedTools: `Write`, `Edit`
- maxTurns: 8
- Body must score dimensions: Security, Correctness, Maintainability, Test Coverage (1–5 each)
- Must block approval (score < 4) for any Critical or High severity issue
...
- disallowedTools: `Write`, `Edit`, `MultiEdit`
- maxTurns: 10
- Body must enumerate: injection, auth/authz, secrets, dependency CVEs, input validation

### Step 4 — Create Skills

Create all skills under `.claude/skills/<name>/SKILL.md`. Prefer `context: fork` +
`agent: Explore` for research-oriented skills, and inline execution for write workflows.

**`plan-feature`** — Feature planning skill
- description triggers: "plan", "design", "how should I implement", "architecture for"
- context: fork, agent: Plan
- allowed-tools: Read, Bash
- Body: produce a structured implementation plan (files to create/modify, test strategy, risks)

**`generate-endpoint`** — REST endpoint generator
- argument-hint: "[resource-name] [HTTP-method]"
- description triggers: "add endpoint", "create route", "new API"
- allowed-tools: Read, Write, Edit, Bash
- Body: follow handler → service → repository → tests pattern from CLAUDE.md

**`write-migration`** — Database migration writer
- argument-hint: "[migration-description]"
- description triggers: "migration", "schema change", "add column", "rename table"
- allowed-tools: Read, Write, Bash
- Body: generate up/down migration, update ORM schema types, add rollback instructions

**`summarize-pr`** — PR summary generator
- argument-hint: "[branch-or-commit-range]"
- description triggers: "summarize PR", "PR description", "what changed in"
- disable-model-invocation: false
- allowed-tools: Bash, Read
- Body: git log + git diff analysis → primary purpose, files changed, 3–5 sentence summary, PR title

**`full-feature`** — End-to-end feature orchestrator
- description triggers: "implement full feature", "build end-to-end", "complete implementation"
- disable-model-invocation: true (explicitly user-triggered only; heavy workflow)
- allowed-tools: Read, Write, Edit, Bash
- Body: invoke `researcher` → create plan → invoke `implementer` → invoke `code-reviewer` → fix issues → re-review until score ≥ 4/5

### Step 5 — Create Hook Scripts

Create all hook scripts under `.claude/hooks/`. Make all scripts executable (`chmod +x`).

**`validate-bash.sh`** — `PreToolUse` on `Bash`
- Read JSON from stdin using `jq`
- Block patterns: `rm -rf /`, `sudo rm`, writing to `/etc`, `chmod 777`, `git push --force`
- Exit 2 to block; emit JSON reason to stderr
- Exit 0 to allow

**`auto-format.sh`** — `PostToolUse` on `Write|Edit|MultiEdit`
- Extract `file_path` from JSON stdin
- Dispatch formatter by extension: `.ts/.tsx` → `prettier --write`, `.py` → `ruff format`, `.go` → `gofmt -w`, `.rs` → `rustfmt`
- Exit 0 unconditionally (format failure is non-blocking)

**`notify-complete.sh`** — `Stop` hook
- Print a brief completion summary to stdout
- Stub out a Slack webhook call using `${SLACK_WEBHOOK_URL}` with a comment explaining how to activate it

### Step 6 — Create `.claude/settings.json`

Generate a complete settings file with:
...
Replace `{{PROJECT_ROOT}}` with the actual project root path. Add MCP server entries
for any databases or external APIs identified from $ARGUMENTS.

### Step 7 — Create `.gitignore` entry

Ensure `.claude/CLAUDE.md` is listed in `.gitignore`. Create a root `.gitignore` if
one does not exist.

### Step 8 — Completion Summary

After all files are created, print a structured summary:
...
### How to Use
- Ask Claude to plan a feature: "plan a user authentication module"
- Generate an endpoint: /generate-endpoint user-profile GET
- Run a full feature cycle: /full-feature "add email verification to signup flow"
- Run security audit: ask Claude to invoke the security-auditor sub-agent

### Next Steps
1. Review and customize CLAUDE.md with your actual build commands
2. Review and customize AGENTS.md with your actual code style rules
3. Set environment variables: SLACK_WEBHOOK_URL (optional), GITHUB_TOKEN (if using GitHub MCP)
4. Add `.claude/CLAUDE.md` entries for any machine-local paths or secret references
```
