...
A library of 20 model-specific prompt templates for the current Anthropic,
Google, and OpenAI model families (refreshed June 2026 against
[current-model-list.md](./current-model-list.md)). Each template contains a
model profile, a fill-in-the-blank prompt scaffold, model-specific prompting
...
2. **If the user named a deprecated/legacy model** (GPT-5.1–5.3, Codex
   variants, Claude Opus 4.5/4.6, Gemini 3 Pro/Flash), recommend the successor
   shown in the index but honor the user's choice if they confirm it.
...
5. **Return the finished prompt in a single fenced code block**, followed by
...
6. **Ask at most one round of clarifying questions**, and only if the intent
...
| [14-anthropic-claude-opus-4-8.md](./14-anthropic-claude-opus-4-8.md) | Claude Opus 4.8 | Long-horizon agentic coding, production code review, deep document/knowledge work, high-stakes writing |
| [02-anthropic-claude-sonnet.md](./02-anthropic-claude-sonnet.md) | Claude Sonnet 4.6 | Production coding agents, multi-tool workflows, balanced cost/quality default |
...
| File | Model | Status |
|---|---|---|
| [01-anthropic-claude-opus.md](./01-anthropic-claude-opus.md) | Claude Opus 4.5 / 4.6 | Active legacy → prefer Opus 4.8 (13/14) |
| [12-anthropic-claude-opus-4-6.md](./12-anthropic-claude-opus-4-6.md) | Claude Opus 4.6 | Active legacy → prefer Opus 4.8 (14) |
...
3. **Code-heavy, multi-file, or long-horizon agentic** → Claude Opus 4.8 or
   GPT-5.5; escalate to Claude Fable 5 only for the hardest autonomous runs.
4. **High volume / cost- or latency-critical** → Claude Haiku 4.5,
   Gemini 3.1 Flash-Lite, or GPT-5.4 nano.
5. **Balanced everyday production work** → Claude Sonnet 4.6, GPT-5.4 (or
   mini), or Gemini 3.5 Flash.
6. **High-stakes professional analysis or writing** → Claude Opus 4.8 or
   GPT-5.5.
7. **Genuine tie** → present the top two with a one-line trade-off and ask.
...
| `01`–`20` `*.md` | Per-model prompt templates (see index above) |
| [user-guide.md](./user-guide.md) | Full selection guidance, template anatomy, prompting best practices |
| [current-model-list.md](./current-model-list.md) | Source of truth for the current model lineup (June 2026) |

**Scope:** general-purpose text/reasoning models only. Claude Mythos
